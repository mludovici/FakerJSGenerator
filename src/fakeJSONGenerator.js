...
export function createRandomPerson() {
  return {
    ...
    address: {
      ...
      "coordinates": [...faker.address.nearbyGPSCoordinate()].map(val => +val),
...
export async function createRandomProductsNew() {
  let res = await fetch('https://dummyjson.com/products?limit=100');
  let data = await res.json();
  return data;
}

export async function createRandomTodos() {
  let res = await fetch('https://dummyjson.com/todos?limit=150');
  let data = await res.json();
  return data;
};
...
export function generateData(amount, fakeDataType) {
  ...
  switch (fakeDataType) {
    ...
    case "companies":
      generatedFakeData = {
        companies: generateFakeDataAmountHelper(amount, createRandomCompany)
      }

      break;
    case "products":
      generatedFakeData = {
        products: generateFakeDataAmountHelper(amount, createRandomProduct)
      }
    ...
    case "payments":
      generatedFakeData = {
        payments: generateFakeDataAmountHelper(amount, createRandomPayment)
      }
    case "todos":
      generatedFakeData = {
        todos: [
